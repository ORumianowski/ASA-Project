...
- *capa_reten* possède 2 outliers. Puisqu'on ne connait pas la stratégie d'échantillonnage, on ne peut pas les exclure.
...
On regarde la matrice de corrélation des variables environnementales (en considerant l'Altitude comme une variable quantitatives).
...
On décide de retirer les variables dont les coefficients de correlation 2 à 2 sont supérieurs à **0.7**.
...
Avant de faire une pCCA, il faut s'assurer que les données ont bien été standardiser, ce qui a été fait dans la partie exploration des données.


## Selection des variables pertinententes avec ordistep

ordistep permet de selectionner les variables à conserver dans le modèle en se basant sur des tests de permutations. Les variables qui ne permettent pas une amélioration significative du modèle sont ainsi retirées.

Dans le code ci-dessous, on construit tout d'abord la formule que le modèle devra utiliser afin de specifier que les données spatiales doivent etre pris en compte dans le modele afin de retirer la part de variance explique par les variables environnementales qui serait liee a de l'autocorrelation spatiale.
...
print("% de la variance expliquée par les variables environnementales")
pourcentage_env = inertie_contrainte / inertie_total *100
pourcentage_env

print("% de la variance expliquée par les variables spatiales")
pourcentage_spatiale = inertie_conditionnelle / inertie_total *100
pourcentage_spatiale

print("% de la variance expliqué par l'environnement contenu dans les 2 premiers axes")
pourcentage_axe1_2_env = ((eigenvalue[[1]] + eigenvalue[[2]]) / sum(eigenvalue)) * 100
pourcentage_axe1_2_env

print("% de l'inertie totale contenue dans les 2 premiers axes")
pourcentage_axe1_2_tot = ((eigenvalue[[1]] + eigenvalue[[2]]) / inertie_total) * 100
pourcentage_axe1_2_tot
```

On verifie que la pCCA est significative avec annova.cca

```{r}
anova.cca(pccaModel_finale)
```

Finalement, les resultats de la pCCA sont significatifs (p=0,001). L'inertie totale de l'analyse est **`r inertie_total`**, l'environnement en explique **11,6%** et les donnees spatiales **8,8%**.

Les 2 premiers axes representent **56,6%** de la variance expliquée par l'environnement (indépendament de l'effet spatiale), et **6,53%** de la variance totale.
...
La representation est refaite avec en montrant les sites et en les colorant selon la communauté vegetale de chaque site (determinée par un expert), qui sont contenue dans les **données associations**.
...
On observe que sans l'altitude le test de Mantel n'est plus significatif (p-value~0.056). Nous poursuivrons tout de même l'analyse avec une pCCA, car la p-value est très proche de la significativité et au vu de l'échantillonnage on s'attend à de l'autocorrélation spatiale. Cela permettra aussi une meilleure comparaison avec l'analyse précendent (avec altitude). 
...
Dans cette 2e analyse, les resultats de la pCCA sont significatifs (p=0,001). L'inertie totale de l'analyse est 8.33, l'environnement en explique **7,8%** et les donnees spatiales **8,8%**.


Les 2 premiers axes representent **64,6%** de la variance expliquée par l'environnement (independament de l'effet spatiale). 
...
Ces différents problèmes avec l'altitude devrait nous pousser à la prudence et donc nous faire choisir l'analyse sans cette variable plutôt que avec. 
